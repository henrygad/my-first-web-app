...
const Blogpostplaceholders = ({ index = 0 }: { index?: number }) => {
    return <div className={`w-full space-y-6 rounded-sm p-4 animate-pulse ${(index % 2 !== 0) ? 'border-y' : ''}`}>
        ...
        <div className="w-full h-20 bg-slate-200 rounded-sm"></div>
        <div className="w-full h-10 bg-slate-200 rounded-sm"></div>
    </div>
};

export default Blogpostplaceholders;
